...
struct 대진대학교: UniversityScrappable {
  ...
  func requestPosts(inCategory category: 대진대학교.Category, inPage page: Int, searchText text: String, _ completion: @escaping (([Post]?, Error?) -> Void)) {
    HTMLParseManager.shared.request(pageURL(inCategory: category, inPage: page, searchText: text), encoding: .eucKR) { doc, error in
      guard let doc = doc else {
        completion(nil, error)
        return
      }
      var posts = [Post]()
      let rows = doc.xpath("//div[2]//tbody//tr//td")
      let links = doc.xpath("//div[2]//tbody//tr//td//a/@href")
